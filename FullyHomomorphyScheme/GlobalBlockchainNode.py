...
class GlobalBlockchainNode(BlockchainNode):
    def __init__(self, blockchain: Blockchain, sleep_time: float = 0.2, traffic_update_interval_in_seconds: int = 10,
        ...
        self.ts_ctx = ts.context(ts.SCHEME_TYPE.BFV, poly_modulus_degree=poly_modulus_degree, plain_modulus=plain_modulus, )
    ...
    def check_and_answer_facilitating_request(self):
        ...
        if latest_block.data["type"] == "request_facilitator":
            self.blockchain.add_block({
                ...
                "public_ctx": b64_enc(self.ts_ctx.serialize(save_public_key=True, save_secret_key=False, save_galois_keys=False, save_relin_keys=True)), # TODO: is relin keys needed?
            })
